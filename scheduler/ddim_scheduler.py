...
class DDIMScheduler:
    def __init__(
        ...
        # 记录最后一个时间步的alpha累积乘积（即训练序列的第一个时间步）
    ...
    def set_timesteps(self, num_inference_steps: int):
        ...
        timesteps = np.array(list(range(0, self.num_train_timesteps, step_ratio)))
    ...
    def step(
        ...
        """
        # 确定当前 timestep 在采样序列中的位置
        step_index = np.where(self.timesteps == timestep)[0].item()
        ...
        # 计算前一个样本
        prev_sample = np.sqrt(alpha_prod_t_prev) * pred_original_sample + pred_sample_direction + sigma_t * noise
